...
func (v *View) RenderSingle(w http.ResponseWriter) {
	...
	for i, name := range templateList {
		...
		path := v.Folder + string(os.PathSeparator) + name + "." + v.Extension
		// if err != nil {
	...
	templates, err := template.New(v.Name).Funcs(pc).ParseFS(templateFS, templateList...)
	// templates, err := template.ParseFS(indexHTML, "templates/index.html.tmpl")
